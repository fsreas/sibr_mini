...
namespace myViewer
{
	...
	public:
		...
		geomId	addMesh( const myViewer::myMesh& mesh );

		/// Add a triangle mesh to the raycast scene, that you will frequently update.
		/// \param mesh the mesh to add
		/// \return the mesh ID or Raycaster::InvalidGeomId if it fails.
		geomId	addDynamicMesh( const myViewer::myMesh& mesh );

		/// Add a triangle mesh to the raycast scene.
		/// \param mesh the mesh to add
		/// \param type the type of mesh
		/// \return the mesh ID or Raycaster::InvalidGeomId if it fails.
		geomId	addGenericMesh( const myViewer::myMesh& mesh, RTCBuildQuality type );

		/// Transform the vertices of a mesh by applying a myViewer::Matrix4f mat.
		...
		void xformRtcMeshOnly(myViewer::myMesh& mesh, geomId mesh_id, myViewer::Matrix4f& mat, myViewer::Vector3f& centerPt, float& maxlen);
		...
		/// \param mesh myViewer::Mesh used by raycaster
		/// \param hit intersection basic information
		/// \return the interpolated normalized normal
		static myViewer::Vector3f smoothNormal(const myViewer::myMesh & mesh, const RayHit & hit);

		/// Interpolate color at a hit (barycentric interpolation), assuming the mesh has colors.
		/// \param mesh myViewer::Mesh used by raycaster
		/// \param hit intersection basic information
		/// \return the interpolated color
		static myViewer::Vector3f smoothColor(const myViewer::myMesh & mesh, const RayHit & hit);
		...
		static myViewer::Vector2f smoothUV(const myViewer::myMesh & mesh, const RayHit & hit);
