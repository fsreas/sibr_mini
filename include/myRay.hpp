...
namespace myViewer
{
	...
	public:
		...
		*/
		Ray( const myViewer::Vector3f& orig = myViewer::Vector3f(0.f, 0.f, 0.f),
			const myViewer::Vector3f& dir = myViewer::Vector3f(0.f, 0.f, -1.f) );

		/** Set the position from where the ray starts.
		\param o the new origin
		*/
		inline void		orig( const myViewer::Vector3f& o );
		
		/// \return the ray origin
		inline const myViewer::Vector3f&	orig( void ) const;
		 ...
		 */
		inline void		dir( const myViewer::Vector3f& d, bool normalizeIt=true );

		/// \return the direction of the ray.
		inline const myViewer::Vector3f&	dir( void ) const;
	...
	private:
		myViewer::Vector3f		_orig;	///< Position from where the ray starts
		myViewer::Vector3f		_dir;	///< Direction where the ray goes
	};
	...
	public:
		...
		*/
		myViewer::Vector3f			interpolateUV( void ) const;

		/// \return the normal of the triangle that was hit.
		inline const myViewer::Vector3f&			normal( void ) const;
	...
	private:
		...
		myViewer::Vector3f	_normal;///< normal of the triangle that was hit
		Primitive	_prim;		///< infos about the primitive that was hit
	};

	///// DEFINITION /////
	
	void		Ray::orig( const myViewer::Vector3f& o ) {
		_orig = o;
	}
	const myViewer::Vector3f&	Ray::orig( void ) const {
		return _orig;
	}

	void		Ray::dir( const myViewer::Vector3f& d, bool normalizeIt) {
		_dir = (normalizeIt)? myViewer::Vector3f(d.normalized()) : d;
	}
	const myViewer::Vector3f&	Ray::dir( void ) const {
		return _dir;
	}
	...
	const myViewer::Vector3f&			RayHit::normal( void ) const {
		return _normal;
	}
