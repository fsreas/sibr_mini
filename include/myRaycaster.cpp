...
namespace myViewer
{
	/*static*/ const Raycaster::geomId		Raycaster::InvalidGeomId = RTC_INVALID_GEOMETRY_ID;
	...
	/*static*/ void Raycaster::rtcErrorCallback(void* userPtr, RTCError code, const char* msg)
	{
		...
		std::cout << "Embree reported the following issue - "
			<< "[" << err << "]'" << msg << "'" << std::endl;
	}
	...
	bool	Raycaster::init(RTCSceneFlags sceneType)
	{
		if (!g_device)
		{
			...
			std::cout << "Initializing Raycaster" << std::endl;

			g_device = std::make_shared<RTCDevice>(rtcNewDevice(NULL));

			if (g_device == nullptr) {
				myViewer_LOG << "Cannot create an embree device : " << rtcGetDeviceError(*g_device.get()) << std::endl;
			}
		...
		if (_scene == nullptr)
			std::cout << "Cannot create an embree scene" << std::endl;
		else {
			//myViewer_LOG << "Embree device and scene created" << std::endl;
			//myViewer_LOG << "Warning Backface culling state : "<< rtcGetDeviceProperty(*g_device, RTC_DEVICE_PROPERTY_BACKFACE_CULLING_ENABLED) << std::endl;
			return true; // Success
		}
		return false; // Fail
	}

	Raycaster::geomId	Raycaster::addMesh(const myViewer::Mesh& mesh)
	{
		return addGenericMesh(mesh, RTC_BUILD_QUALITY_HIGH);
	}

	Raycaster::geomId	Raycaster::addDynamicMesh(const myViewer::Mesh& mesh)
	{
		return addGenericMesh(mesh, RTC_BUILD_QUALITY_LOW);
	}

	Raycaster::geomId	Raycaster::addGenericMesh(const myViewer::Mesh& mesh, RTCBuildQuality type)
	{
		if (init() == false)
			return Raycaster::InvalidGeomId;

		const std::vector<Vector3f>& vertices = mesh.vertices();
		const std::vector<Vector3u>& triangles = mesh.triangles();
	...
	void	Raycaster::xformRtcMeshOnly(myViewer::Mesh& mesh, geomId mesh_id, myViewer::Matrix4f& mat, myViewer::Vector3f& centerPt, float& maxlen)
	{
		struct Vertex { float x, y, z, a; };
		...
		myViewer::Vector4f averagePt = myViewer::Vector4f(0, 0, 0, 1);
		maxlen = 0;

		const std::vector<Vector3f>& vertices = mesh.vertices();
		//const myViewer::Mesh::Normals& normals = mesh.normals();
		for (uint i = 0; i < mesh.vertices().size(); ++i)
		{
			myViewer::Vector4f v;
			...
			averagePt += v;
			float d = myViewer::Vector3f(myViewer::Vector4f(averagePt / (float)((i == 0) ? 1 : i)).xyz() - v.xyz()).norm();
			if (d > maxlen)
				maxlen = d;
		}

		myViewer::Vector4f cp = averagePt / (float)mesh.vertices().size();
		centerPt = myViewer::Vector3f(cp[0], cp[1], cp[2]);
	...
	bool	Raycaster::hitSomething(const Ray& inray, float minDist)
	{
		...
		if (init() == false)
			myViewer_ERR << "cannot initialize embree, failed cast rays." << std::endl;
		else
		{
	...
	RayHit	Raycaster::intersect(const Ray& inray, float minDist)
	{
		...
		myViewer::Vector3f normal = myViewer::Vector3f(rh.hit.Ng_x, rh.hit.Ng_y, rh.hit.Ng_z);
		...
		for (int r = 0; r < 8; r++) {
			if (valid8[r])
				res[r] = {
					...
					myViewer::Vector3f(rh.hit.Ng_x[r], rh.hit.Ng_y[r], rh.hit.Ng_z[r]),
					RayHit::Primitive{
#ifdef myViewer_OS_WINDOWS
	...
	myViewer::Vector3f Raycaster::smoothNormal(const myViewer::Mesh& mesh, const RayHit& hit)
	{
		if (!mesh.hasNormals()) {
			myViewer_ERR << " cannot compute smoothed normals if the mesh does not have normals " << std::endl;
		}
		const myViewer::Mesh::Normals& normals = mesh.normals();
		const myViewer::Vector3u& tri = mesh.triangles()[hit.primitive().triID];
	...
	myViewer::Vector3f Raycaster::smoothColor(const myViewer::Mesh& mesh, const RayHit& hit)
	{
		if (!mesh.hasColors()) {
			myViewer_ERR << " cannot compute smoothed color if the mesh does not have colors " << std::endl;
		}
		const myViewer::Mesh::Colors& colors = mesh.colors();
		const myViewer::Vector3u& tri = mesh.triangles()[hit.primitive().triID];
	...
	myViewer::Vector2f Raycaster::smoothUV(const myViewer::Mesh& mesh, const RayHit& hit)
	{
		if (!mesh.hasTexCoords()) {
			myViewer_ERR << " cannot compute UV if the mesh does not have texcoords " << std::endl;
		}
		const myViewer::Mesh::UVs& uvs = mesh.texCoords();
		const myViewer::Vector3u& tri = mesh.triangles()[hit.primitive().triID];
